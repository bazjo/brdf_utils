...
for incidence_angle=45 //workaround described in documentation; set as 5:5:80 to export all incidence angles
...
for incidence_angle=45 //workaround described in documentation; set as 5:5:80 to export all incidence angles
    for theta=5:5:80
        for scatter_azimuth=0:10:350
            ...
            mputstr(ascii(9), surface_property)
            mputstr(string(irradiance_map_theta(theta/5,incidence_angle/5)/1000000), surface_property) //BRDF (counts scaled to appropriate order of magnitude)
            mputstr(ascii(9), surface_property)
